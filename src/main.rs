...
use proto::header::ResCode;
use proto::packet::Packet;
use proto::query::Query;
use proto::question::Question;
use proto::{BytePacketBuffer, Result};

const ROOT_SERVER: &str = "192.5.5.241";

fn recursive_lookup(qname: &str, qtype: Query) -> Result<Packet> {
    ...
    loop {
        ...
        if !response.answers.is_empty() && response.header.rescode == ResCode::NoError {
            return Ok(response);
        }

        // We might also get a `NXDOMAIN` reply, which is the authoritative name servers
        // way of telling us that the name doesn't exist.
        if response.header.rescode == ResCode::NameError {
            return Ok(response);
        }
        ...
        // name server.
        let recursive_response = recursive_lookup(&new_ns_name, Query::A)?;
...
fn lookup(qname: &str, qtype: Query, server: (Ipv4Addr, u16)) -> Result<Packet> {
...
fn handle_query(socket: &UdpSocket) -> Result<()> {
    ...
    if let Some(question) = request.questions.pop() {
        ...
        if let Ok(result) = recursive_lookup(&question.name, question.qtype) {
            ...
            packet.header.rescode = ResCode::ServerFailure;
        }
    }
    ...
    // to indicate that the sender made something wrong.
        ...
        packet.header.rescode = ResCode::FormatError;
    }
